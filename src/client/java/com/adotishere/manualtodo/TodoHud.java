...
public final class TodoHud {
    ...
    private static void renderList(
        ...
        drawScaledText(graphics, font, count, countRight - Math.round(font.width(count) * list.fontScale), 7, opaque(theme.mutedText), false, list.fontScale);
            ...
            for (int taskIndex = 0; taskIndex < list.tasks.size(); taskIndex++) {
                ...
                if (task.incremental) {
                    ...
                    if (mode == Mode.EDITOR) {
                        ...
                        drawScaledText(graphics, font, progress, progressX, rowY + 4, opaque(theme.mutedText), false, list.fontScale);
